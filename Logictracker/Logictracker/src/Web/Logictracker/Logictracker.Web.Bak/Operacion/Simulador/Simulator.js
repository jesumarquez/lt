...
DDSimulator.prototype.moveToPointDriving_ = function(loc, heading) {
  ...
  var desiredRange = Math.max(50.0, this.currentSpeed * 10);
  var curTilt = oldLa.getTilt();
...
DDSimulator.prototype.tick_ = function() {
  ...
  if (!this.beforeSegmentDistance_) this.beforeSegmentDistance_ = 0.0;
  if (!this.beforeSegmentDuration_) this.beforeSegmentDuration_ = 0.0;
  if (!this.beforeSegmentSpeed_) this.beforeSegmentSpeed_ = 0.0;
  ...
  while (this.pathIndex_ < this.path.length - 1 && this.segmentTime_ >= segmentDuration) {
    ...
    this.beforeSegmentDuration_ += this.path[this.pathIndex_].duration;
    this.beforeSegmentSpeed_ = this.path[this.pathIndex_].speed;
    this.segmentDistance_ = 0;
  ...
  if (segmentDuration) {
    this.segmentDistance_ = this.path[this.pathIndex_].distance * Math.min(1.0, this.segmentTime_ / segmentDuration);
    this.currentSpeed = this.beforeSegmentSpeed_ + ((this.path[this.pathIndex_].speed - this.beforeSegmentSpeed_) * Math.min(1.0, this.segmentTime_ / segmentDuration));
    //this.currentSpeed = this.path[this.pathIndex_].speed;//this.path[this.pathIndex_].distance / segmentDuration;
  } else {
  ...
  this.totalDuration = this.beforeSegmentDuration_ + this.segmentTime_;
...
DDSimulator.prototype.goTo = function(time)
{
   
    this.beforeSegmentDuration_ = 0;
    this.beforeSegmentDistance_ = 0;
    for(var i = 0; i < this.path.length - 1; i++)
    {
        var path = this.path[i];
        this.pathIndex_ = i;
        this.currentStep_ = path.step;
        if(this.beforeSegmentDuration_ >= time) break;
        
        this.beforeSegmentDuration_ += path.duration;
        this.beforeSegmentDistance_ += path.distance;
        this.beforeSegmentSpeed_ = this.path[this.pathIndex_].speed;
    }
