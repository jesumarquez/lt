var slideShow=function(){
	var bxs,bxe,fxs,fxe,ys,ye,ta,ia,ie,st,ss,ft,fs,xp,yp,ci,t,tar,tarl,time;
	ta=document.getElementById(thumbid); ia=document.getElementById(imgid);
	t=ta.getElementsByTagName('li'); dt=document.getElementById(timeid);ie=document.all?true:false;
	...
	return{
		init:function(){
			...
			len=t.length;tar=[];time=[];
			for(i=0;i<len;i++){
				var id=t[i].getAttribute("value"); tar[i]=id;
				var tm=t[i].getAttribute("time"); time[i]=tm;
		...
		gettime:function(id){
		    for(var i=0;i<tar.length;i++){if(tar[i]==id){return time[i];}}
		},
		nav:function(d){
			var c=0;
