...
namespace Logictracker.Web.App_Services
{
    ...
    public class OperationService : BaseWebService
    {
        ...
        [WebMethod]
        public Respuesta<BaseClasses.Handlers.Distancia[]> GetMovilesCercanos(double latitud, double longitud, int cantidad, string empresa, string linea)
        {
            try
            {
                var emp = DAOFactory.EmpresaDAO.FindByCodigo(empresa);
                if (emp == null) return Respuesta<BaseClasses.Handlers.Distancia[]>.CreateError("Empresa " + empresa + " no encontrada");
                var lin = DAOFactory.LineaDAO.FindByCodigo(emp.Id, linea);
                if (lin == null) return Respuesta<BaseClasses.Handlers.Distancia[]>.CreateError("Linea " + linea + " no encontrada");
                if (!lin.Interfaceable) return Respuesta<BaseClasses.Handlers.Distancia[]>.CreateError("La Linea " + linea + " no acepta consultas de esta interface");

                var coches = DAOFactory.CocheDAO.FindList(new[] { emp.Id }, new[] { lin.Id });
                if (!coches.Any())
                    return Respuesta<BaseClasses.Handlers.Distancia[]>.CreateError("No se encontraron móviles para " + empresa + " - " + linea);
                ...
                var distancias = pois.Select(p => new BaseClasses.Handlers.Distancia { Interno = p.Interno, Metros = p.Distancia }).ToArray();
                return Respuesta<BaseClasses.Handlers.Distancia[]>.Create(distancias);
            }
            catch (Exception ex)
            {
                return Respuesta<BaseClasses.Handlers.Distancia[]>.CreateError(ex.Message);
            }
        }
        ...
        [WebMethod]
        public Respuesta<BaseClasses.Handlers.Posicion[]> GetLocationByCompany(string company)
        {
            return GetLocationByBranch(company, string.Empty);
        }
        [WebMethod]
        public Respuesta<BaseClasses.Handlers.Posicion[]> GetLocationByBranch(string company, string branch)
        {
            try
            {
                var time = CheckTime<BaseClasses.Handlers.Posicion[]>("GetPosiciones", company, branch);
                if (!time.RespuestaOk) return time;

                var emp = DAOFactory.EmpresaDAO.FindByCodigo(company);
                if (emp == null) return Respuesta<BaseClasses.Handlers.Posicion[]>.CreateError("Empresa " + company + " no encontrada");
                Linea lin = null;
                if (branch != string.Empty)
                {
                    lin = DAOFactory.LineaDAO.FindByCodigo(emp.Id, branch);
                    if (lin == null) return Respuesta<BaseClasses.Handlers.Posicion[]>.CreateError("Linea " + branch + " no encontrada");
                    if (!lin.Interfaceable) return Respuesta<BaseClasses.Handlers.Posicion[]>.CreateError("La Linea " + branch + " no acepta consultas de esta interface");
                }
                ...
                return Respuesta<BaseClasses.Handlers.Posicion[]>.Create(posiciones.Where(p => p != null).ToArray());
            }
            catch (Exception ex)
            {
                return Respuesta<BaseClasses.Handlers.Posicion[]>.CreateError(ex.Message);
            }
        }

        [WebMethod]
        public Respuesta<BaseClasses.Handlers.Posicion> GetLocationByVehicle(string company, string branch, string vehicle)
        {
            try
            {
                var time = CheckTime<BaseClasses.Handlers.Posicion>("GetPosiciones", company, branch);
                ...
                if (coche == null) return Respuesta<BaseClasses.Handlers.Posicion>.CreateError("No se encontro el vehiculo " + vehicle);

                return Respuesta<BaseClasses.Handlers.Posicion>.Create(GetPosicion(coche));
            }
            catch (Exception ex)
            {
                return Respuesta<BaseClasses.Handlers.Posicion>.CreateError(ex.Message);
        ...
        private new BaseClasses.Handlers.Posicion GetPosicion(Coche coche)
        {
            var pos = DAOFactory.LogPosicionDAO.GetLastOnlineVehiclePosition(coche);
            if (pos == null) return null;
            return new BaseClasses.Handlers.Posicion
            {
