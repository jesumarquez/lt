...
namespace Quartz
{
	...
	public class NthIncludedDayTrigger : Trigger
	{
		...
		public virtual string FireAtTime
		{
			...
			set
			{
				try
				{
                    ...
                    var newFireHour = Int32.Parse(components[0], CultureInfo.InvariantCulture);
					if (components[1].Length != 2)
					{
						// minutes must be in two digit format
						throw new Exception();
					}
                    var newFireMinute = Int32.Parse(components[1], CultureInfo.InvariantCulture);
					var newFireSecond = 0;
		...
		{
			calendar = cal;
			var dt = StartTimeUtc.AddMilliseconds(-1*1000);
		...
		{
			var instruction = MisfireInstruction;
