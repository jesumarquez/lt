...
namespace Quartz.Impl.AdoJobStore
{
    ...
    public class StdAdoDelegate : StdAdoConstants, IDriverDelegate
    {
		...
		*/
        ...
        public virtual Key[] SelectMisfiredTriggers(ConnectionAndTransactionHolder conn, long ts)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectMisfiredTriggers)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    ...
                    while (rs.Read())
                    {
                        var triggerName = GetString(rs[ColumnTriggerName]);
                        var groupName = GetString(rs[ColumnTriggerGroup]);
                        list.Add(new Key(triggerName, groupName));
                    }
                    var oArr = list.ToArray();
                    var kArr = new Key[oArr.Length];
        ...
        public virtual Key[] SelectTriggersInState(ConnectionAndTransactionHolder conn, string state)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggersInState)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    ...
                    var sArr = (Key[])list.ToArray(typeof(Key));
                    return sArr;
                }
            }
        }
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectMisfiredTriggersInStates)))
            {
        ...
        public virtual Trigger[] SelectTriggersForRecoveringJobs(ConnectionAndTransactionHolder conn)
        {
            ...
            using (
                var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectInstancesRecoverableFiredTriggers)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    var dumId = DateTime.UtcNow.Ticks;
                    ...
                    while (rs.Read())
                    {
                        ...
                        var firedTimeInTicks = Convert.ToInt64(rs[ColumnFiredTime], CultureInfo.InvariantCulture);
                        var priority = Convert.ToInt32(rs[ColumnPriority], CultureInfo.InvariantCulture);
                        var firedTime = new DateTime(firedTimeInTicks);
                        var rcvryTrig =
            ...
            {
                var jd = SelectTriggerJobDataMap(conn, trigger.Name, trigger.Group);
            ...
            var oArr = list.ToArray();
            var tArr = new Trigger[oArr.Length];
        ...
        {
            var baos = SerializeJobData(job.JobDataMap);
            ...
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertJobDetail)))
            {
                ...
                if (insertResult > 0)
                {
                    var jobListeners = job.JobListenerNames;
                    for (var i = 0; jobListeners != null && i < jobListeners.Length; i++)
                    {
        ...
        {
            var idx = jobType.AssemblyQualifiedName.IndexOf(',');
            // find next
            idx = jobType.AssemblyQualifiedName.IndexOf(',', idx + 1);

            var retValue = jobType.AssemblyQualifiedName.Substring(0, idx);
                                                      ...
                                                      string groupName)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggersForJob)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    ...
                    while (rs.Read())
                    {
                        var trigName = GetString(rs[ColumnTriggerName]);
                        var trigGroup = GetString(rs[ColumnTriggerGroup]);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlDeleteJobListeners)))
            {
                AddCommandParameter(cmd, 1, "jobName", jobName);
                AddCommandParameter(cmd, 2, "jobGroup", groupName);
                return cmd.ExecuteNonQuery();
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectJobStateful)))
            {
                AddCommandParameter(cmd, 1, "jobName", jobName);
                AddCommandParameter(cmd, 2, "jobGroup", groupName);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertJobListener)))
            {
                AddCommandParameter(cmd, 1, "jobName", job.Name);
                AddCommandParameter(cmd, 2, "jobGroup", job.Group);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectJobListeners)))
            {
                var list = new ArrayList();
        ...
        {
            IDictionary map;
            var properties = (NameValueCollection)GetJobDetailFromBlob(rs, idx);
            if (properties == null)
            {
                return null;
            }
        ...
        public virtual string[] SelectJobGroups(ConnectionAndTransactionHolder conn)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectJobGroups)))
            {
                using (var rs = cmd.ExecuteReader())
                {
                    ...
                    var oArr = list.ToArray();
                    var sArr = new string[oArr.Length];
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectJobsInGroup)))
            {
                AddCommandParameter(cmd, 1, "jobGroup", groupName);
                                         ...
                                         JobDetail jobDetail)
        {
            ...
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertTrigger)))
            {
                ...
                if (insertResult > 0)
                {
                    var trigListeners = trigger.TriggerListenerNames;
                    for (var i = 0; trigListeners != null && i < trigListeners.Length; i++)
                    {
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertSimpleTrigger)))
            {
                AddCommandParameter(cmd, 1, "triggerName", trigger.Name);
                AddCommandParameter(cmd, 2, "triggerGroup", trigger.Group);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertCronTrigger)))
            {
                AddCommandParameter(cmd, 1, "triggerName", trigger.Name);
                AddCommandParameter(cmd, 2, "triggerGroup", trigger.Group);
        ...
        public virtual int InsertBlobTrigger(ConnectionAndTransactionHolder conn, Trigger trigger)
        {
            ...
            {
                // update the blob
                var buf = SerializeObject(trigger);
                AddCommandParameter(cmd, 1, "triggerName", trigger.Name);
                AddCommandParameter(cmd, 2, "triggerGroup", trigger.Group);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlUpdateSimpleTrigger)))
            {
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlUpdateCronTrigger)))
            {
        ...
        public virtual int UpdateBlobTrigger(ConnectionAndTransactionHolder conn, Trigger trigger)
        {
            ...
            {
                // update the blob
                var os = SerializeObject(trigger);
        ...
        public virtual int UpdateTriggerGroupStateFromOtherState(ConnectionAndTransactionHolder conn, string groupName,
                                                                 ...
                                                                 string oldState)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlTriggerGroupStateFromState)))
            {
                AddCommandParameter(cmd, 1, "newState", newState);
                AddCommandParameter(cmd, 2, "triggerGroup", groupName);
        ...
        public virtual int DeleteTriggerListeners(ConnectionAndTransactionHolder conn, string triggerName,
                                                  string groupName)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlDeleteTriggerListeners)))
            {
                AddCommandParameter(cmd, 1, "triggerName", triggerName);
                AddCommandParameter(cmd, 2, "triggerGroup", groupName);
                return cmd.ExecuteNonQuery();
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlInsertTriggerListener)))
            {
                AddCommandParameter(cmd, 1, "triggerName", trigger.Name);
                AddCommandParameter(cmd, 2, "triggerGroup", trigger.Group);
                AddCommandParameter(cmd, 3, "listener", listener);
        ...
        public virtual String[] SelectTriggerListeners(ConnectionAndTransactionHolder conn, string triggerName,
                                                       string groupName)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggerListeners)))
            {
                AddCommandParameter(cmd, 1, "triggerName", triggerName);
                AddCommandParameter(cmd, 2, "triggerGroup", groupName);
                using (var rs = cmd.ExecuteReader())
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlDeleteSimpleTrigger)))
            {
                AddCommandParameter(cmd, 1, "triggerName", triggerName);
                AddCommandParameter(cmd, 2, "triggerGroup", groupName);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlDeleteCronTrigger)))
            {
                AddCommandParameter(cmd, 1, "triggerName", triggerName);
                AddCommandParameter(cmd, 2, "triggerGroup", groupName);
        ...
        public virtual Trigger[] SelectTriggersForJob(ConnectionAndTransactionHolder conn, string jobName,
                                                      string groupName)
        {
            var triggerIdentifiers = new ArrayList();
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggersForJob)))
            {
                AddCommandParameter(cmd, 1, "jobName", jobName);
                AddCommandParameter(cmd, 2, "jobGroup", groupName);
            ...
            var trigList = new ArrayList();
            foreach (Pair p in triggerIdentifiers)
            {
                var t = SelectTrigger(conn, (string)p.First, (string)p.Second);
                if (t != null)
                {
                    trigList.Add(t);
                }
        ...
        public virtual Trigger[] SelectTriggersForCalendar(ConnectionAndTransactionHolder conn, string calName)
        {
            ...
            {
                var triggers = new NameValueCollection();
        ...
        public virtual IList SelectStatefulJobsOfTriggerGroup(ConnectionAndTransactionHolder conn, string groupName)
        {
            ...
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectStatefulJobsOfTriggerGroup)))
            {
                AddCommandParameter(cmd, 1, "jobGroup", groupName);
        ...
        public virtual Trigger SelectTrigger(ConnectionAndTransactionHolder conn, string triggerName, string groupName)
        {
            ...
            var volatility = false;
            string description = null;
            var triggerType = "";
            string calendarName = null;
            var misFireInstr = Int32.MinValue;
            ...
            var startTimeD = DateTime.MinValue;
            ...
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTrigger)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    if (rs.Read())
                    {
                        ...
                        var nextFireTime = Convert.ToInt64(rs[ColumnNextFireTime], CultureInfo.InvariantCulture);
                        var prevFireTime = Convert.ToInt64(rs[ColumnPreviousFireTime], CultureInfo.InvariantCulture);
                        triggerType = GetString(rs[ColumnTriggerType]);
                        var startTime = Convert.ToInt64(rs[ColumnStartTime], CultureInfo.InvariantCulture);
                        var endTime = Convert.ToInt64(rs[ColumnEndTime], CultureInfo.InvariantCulture);
                        ...
                        {
                            using (var cmd2 = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectSimpleTrigger)))
                            {
                                AddCommandParameter(cmd2, 1, "triggerName", triggerName);
                                AddCommandParameter(cmd2, 2, "triggerGroup", groupName);
                                using (var rs2 = cmd2.ExecuteReader())
                                {
                                    if (rs2.Read())
                                    {
                                        var repeatCount = Convert.ToInt32(rs2[ColumnRepeatCount], CultureInfo.InvariantCulture);
                                        var repeatInterval = Convert.ToInt64(rs2[ColumnRepeatInterval], CultureInfo.InvariantCulture);
                                        var timesTriggered = Convert.ToInt32(rs2[ColumnTimesTriggered], CultureInfo.InvariantCulture);
                        ...
                        {
                            using (var cmd2 = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectCronTriggers)))
                            {
                                AddCommandParameter(cmd2, 1, "triggerName", triggerName);
                                AddCommandParameter(cmd2, 2, "triggerGroup", groupName);
                                using (var rs2 = cmd2.ExecuteReader())
                                {
                                    if (rs2.Read())
                                    {
                                        var cronExpr = GetString(rs2[ColumnCronExpression]);
                                        var timeZoneId = GetString(rs2[ColumnTimeZoneId]);
                                                         ...
                                                         string groupName)
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggerStatus)))
            {
                ...
                using (var rs = cmd.ExecuteReader())
                {
                    if (rs.Read())
                    {
                        var state = GetString(rs[ColumnTriggerState]);
                        var nextFireTime = Convert.ToInt64(rs[ColumnNextFireTime], CultureInfo.InvariantCulture);
                        var jobName = GetString(rs[ColumnJobName]);
                        var jobGroup = GetString(rs[ColumnJobGroup]);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectTriggersInGroup)))
            {
        ...
        {
            var baos = SerializeObject(calendar);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectNextTriggerToAcquire)))
            {
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlDeleteVolatileFiredTriggers)))
            {
        ...
        public virtual IList SelectSchedulerStateRecords(ConnectionAndTransactionHolder conn, string instanceName)
        {
            ...
            using (var rs = cmd.ExecuteReader())
            {
                while (rs.Read())
                {
                    var rec = new SchedulerStateRecord();
                    rec.SchedulerInstanceId = GetString(rs[ColumnInstanceName]);
        ...
        private byte[] SerializeProperties(JobDataMap data)
        {
            ...
            if (null != data)
            {
                var properties = ConvertToProperty(data.WrappedMap);
        ...
        protected internal virtual NameValueCollection ConvertToProperty(IDictionary data)
        {
            ...
            {
                var key = entry.Key;
                var val = entry.Value == null ? string.Empty : entry.Value;
        ...
        protected internal virtual object GetObjectFromBlob(IDataReader rs, int colIndex)
        {
            ...
            var data = ReadBytesFromBlob(rs, colIndex);
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectVolatileTriggers)))
            {
        ...
        {
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectVolatileJobs)))
            {
                AddCommandParameter(cmd, 1, "isVolatile", GetDbBooleanValue(true));
                using (var dr = cmd.ExecuteReader())
                {
                    ...
                    while (dr.Read())
                    {
                        var triggerName = (string)dr[ColumnJobName];
                        var groupName = (string)dr[ColumnJobGroup];
                        list.Add(new Key(triggerName, groupName));
                    }
                    var oArr = list.ToArray();
                    var kArr = new Key[oArr.Length];
        ...
        public virtual ISet SelectPausedTriggerGroups(ConnectionAndTransactionHolder conn)
        {
            ...
            using (var cmd = PrepareCommand(conn, ReplaceTablePrefix(SqlSelectPausedTriggerGroups)))
            {
                using (var dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        var groupName = (string)dr[ColumnTriggerGroup];
