    ...
    public abstract class JobStoreSupport : AdoConstants, IJobStore
    {
            ...
            get
            {
                if (driverDelegate == null)
                {
                    try
                    {
                        ...
                        {
                            var ctorParamTypes =
                                new[] { typeof(ILog), typeof(String), typeof(String), typeof(IDbProvider) };
                            ctor = delegateType.GetConstructor(ctorParamTypes);
        ...
        protected virtual void CleanVolatileTriggerAndJobs(ConnectionAndTransactionHolder conn)
        {
            try
            {
                ...
                var volatileTriggers = Delegate.SelectVolatileTriggers(conn);
                var volatileJobs = Delegate.SelectVolatileJobs(conn);

                for (var i = 0; i < volatileTriggers.Length; i++)
                {
                ...
                for (var i = 0; i < volatileJobs.Length; i++)
                {
        ...
        protected virtual void RecoverJobs(ConnectionAndTransactionHolder conn)
        {
            try
            {
                ...
                var recoveringJobTriggers = Delegate.SelectTriggersForRecoveringJobs(conn);
                Log.Info("Recovering " + recoveringJobTriggers.Length +
                         " jobs that were in-progress at the time of the last shut-down.");

                for (var i = 0; i < recoveringJobTriggers.Length; ++i)
                {
                ...
                var ct = Delegate.SelectTriggersInState(conn, StateComplete);
                for (var i = 0; ct != null && i < ct.Length; i++)
                {
                ...
                // clean up any fired trigger entries
                var n = Delegate.DeleteFiredTriggers(conn);
                                                                     ...
                                                                     bool recovering)
        {
            ...
            // triggers right away.
            var maxMisfiresToHandleAtATime = (recovering) ? -1 : MaxMisfiresToHandleAtATime;
            ...
            {
                var trig = RetrieveTrigger(conn, triggerKey.Name, triggerKey.Group);
                ...
                var nextTime = trig.GetNextFireTimeUtc();
                                                              ...
                                                              bool forceState)
        {
            try
            {
                var trig = Delegate.SelectTrigger(conn, triggerName, groupName);
                                                 ...
                                                 bool replaceExisting)
        {
            ...
            var existingJob = JobExists(conn, newJob.Name, newJob.Group);
            try
            {
                if (existingJob)
                {
                                                     ...
                                                     bool recovering)
        {
            ...
            var existingTrigger = TriggerExists(conn, newTrigger.Name, newTrigger.Group);
            ...
            try
            {
                if (!forceState)
                {
                    var shouldBepaused = Delegate.IsTriggerGroupPaused(conn, newTrigger.Group);
            ...
            {
                var message = String.Format("Couldn't store trigger '{0}' for '{1}' job: {2}", newTrigger.Name, newTrigger.JobName, e.Message);
                                                  ...
                                                  string groupName, bool activeDeleteSafe)
        {
            try
            {
                var jobTriggers = Delegate.SelectTriggerNamesForJob(conn, jobName, groupName);

                for (var i = 0; i < jobTriggers.Length; ++i)
                {
                                                         ...
                                                         string groupName)
        {
            try
            {
                var job = Delegate.SelectJobDetail(conn, jobName, groupName, TypeLoadHelper);
                var listeners = Delegate.SelectJobListeners(conn, jobName, groupName);
                                                      ...
                                                      string groupName)
        {
            ...
            try
            {
                ...
                var job = Delegate.SelectJobForTrigger(conn, triggerName, groupName, TypeLoadHelper);

                removedTrigger = DeleteTriggerAndChildren(conn, triggerName, groupName);

                if (null != job && !job.Durable)
                {
                    var numTriggers = Delegate.SelectNumTriggersForJob(conn, job.Name, job.Group);
                    if (numTriggers == 0)
                    {
                                                       ...
                                                       string groupName, Trigger newTrigger)
        {
            try
            {
                ...
                var removedTrigger = DeleteTriggerAndChildren(conn, triggerName, groupName);
                                                           ...
                                                           string groupName)
        {
            try
            {
                var trigger = Delegate.SelectTrigger(conn, triggerName, groupName);
                if (trigger == null)
                {
                    return null;
                }
                ...
                var listeners = Delegate.SelectTriggerListeners(conn, triggerName, groupName);
                for (var i = 0; i < listeners.Length; ++i)
                {
                                           ...
                                           string triggerName, string groupName)
        {
            try
            {
                var ts = Delegate.SelectTriggerState(conn, triggerName, groupName);
                                                      ...
                                                      ICalendar calendar, bool replaceExisting, bool updateTriggers)
        {
            try
            {
                var existingCal = CalendarExists(conn, calName);
                ...
                if (existingCal)
                {
                    ...
                    if (updateTriggers)
                    {
                        var trigs = Delegate.SelectTriggersForCalendar(conn, calName);
                                                              ...
                                                              SchedulingContext ctxt, string calName)
        {
            ...
            var cal = Clustered ? null : (ICalendar)calendarCache[calName];
            if (cal != null)
            {
                return cal;
            }
                                         ...
                                         string groupName)
        {
            try
            {
                var oldState = Delegate.SelectTriggerState(conn, triggerName, groupName);
        ...
        protected class PauseJobCallback : CallbackSupport, IVoidTransactionCallback
        {
            ...
            public void Execute(ConnectionAndTransactionHolder conn)
            {
                var triggers = js.GetTriggersForJob(conn, ctxt, jobName, groupName);
        ...
        protected class PauseJobGroupCallback : CallbackSupport, IVoidTransactionCallback
        {
            ...
            public void Execute(ConnectionAndTransactionHolder conn)
            {
                var jobNames = js.GetJobNames(conn, ctxt, groupName);

                for (var i = 0; i < jobNames.Length; i++)
                {
                    var triggers = js.GetTriggersForJob(conn, ctxt, jobNames[i], groupName);
                    for (var j = 0; j < triggers.Length; j++)
                    {
                        js.PauseTrigger(conn, ctxt, triggers[j].Name, triggers[j].Group);
                    }
            ...
            string jobGroupName, string currentState)
        {
            ...
            try
            {
                var lst = Delegate.SelectFiredTriggerRecordsByJob(conn,
                                                                    jobName, jobGroupName);

                if (lst.Count > 0)
                {
                    var rec = (FiredTriggerRecord)lst[0];
                                                                 ...
                                                                 string groupName)
        {
            try
            {
                ...
                var lst = Delegate.SelectFiredTriggerRecordsByJob(conn, jobName, groupName);
        ...
        */
                                          ...
                                          string triggerName, string groupName)
        {
            try
            {
                var status = Delegate.SelectTriggerStatus(conn, triggerName, groupName);
                ...
                var newState = CheckBlockedState(conn, ctxt, status.JobKey.Name, status.JobKey.Group, StateWaiting);
                                               ...
                                               string groupName)
        {
            try
            {
                ...
                var trigNames = Delegate.SelectTriggersInGroup(conn, groupName);

                for (var i = 0; i < trigNames.Length; i++)
                {
        ...
        {
            var names = GetTriggerGroupNames(conn, ctxt);
                                                              ...
                                                              SchedulingContext ctxt, DateTime noLaterThan)
        {
            do
            {
                try
                {
                    var triggerKey = Delegate.SelectTriggerToAcquire(conn, noLaterThan, MisfireTime);
                    ...
                    var rowsUpdated = Delegate.UpdateTriggerStateFromOtherState(
                        conn,
                    ...
                    var nextTrigger = RetrieveTrigger(conn, ctxt, triggerKey.Name, triggerKey.Group);
                                                                   ...
                                                                   Trigger trigger)
        {
            ...
            try
            {
                ...
                var state = Delegate.SelectTriggerState(conn, trigger.Name, trigger.Group);
            ...
            try
            {
                if (triggerInstCode == SchedulerInstruction.DeleteTrigger)
                {
                    if (!trigger.GetNextFireTimeUtc().HasValue)
                    {
                        ...
                        var stat = Delegate.SelectTriggerStatus(conn, trigger.Name, trigger.Group);
        ...
        protected RecoverMisfiredJobsResult DoRecoverMisfires()
        {
            ...
            var conn = GetNonManagedTXConnection();
            try
            {
                ...
                var misfireCount = (DoubleCheckLockMisfireHandler)
                                       ?
        ...
        protected virtual IList FindFailedInstances(ConnectionAndTransactionHolder conn)
        {
            try
            {
                var failedInstances = new ArrayList();
                var foundThisScheduler = false;

                var states = Delegate.SelectSchedulerStateRecords(conn, null);
        ...
        private IList FindOrphanedFailedInstances(ConnectionAndTransactionHolder conn, IList schedulerStateRecords)
        {
            ...
            var allFiredTriggerInstanceNames = Delegate.SelectFiredTriggerInstanceNames(conn);
        ...
        {
            var passed = DateTime.UtcNow - lastCheckin;
            var ts = rec.CheckinInterval > passed ? rec.CheckinInterval : passed;
        ...
        {
            var failedInstances = FindFailedInstances(conn);
            try
            {
        ...
        protected virtual void ClusterRecover(ConnectionAndTransactionHolder conn, IList failedInstances)
        {
            ...
            {
                var recoverIds = DateTime.UtcNow.Ticks;
                    ...
                    foreach (SchedulerStateRecord rec in failedInstances)
                    {
                        ...
                        foreach (FiredTriggerRecord ftRec in firedTriggerRecs)
                        {
                            ...
                            else if (ftRec.JobRequestsRecovery)
                            {
                                ...
                                {
                                    var tempAux = new DateTime(ftRec.FireTimestamp);
                                    var rcvryTrig =
                                        new SimpleTrigger(
                                    ...
                                    var jd = Delegate.SelectTriggerJobDataMap(conn, tKey.Name, tKey.Group);
                        ...
                        foreach (Key triggerKey in triggerKeys)
                        {
                                    ...
                                    StateComplete))
                            {
                                ...
                                {
                                    var schedulingContext = new SchedulingContext();
        ...
        protected virtual void CommitConnection(ConnectionAndTransactionHolder cth, bool openNewTransaction)
        {
            if (cth.Transaction != null)
            {
                try
                {
                    var il = cth.Transaction.IsolationLevel;
        ...
        internal class ClusterManager : QuartzThread
        {
            ...
            public override void Run()
            {
                while (!shutdown)
                {
                    if (!shutdown)
                    {
                        var timeToSleep = jobStoreSupport.ClusterCheckinInterval;
                        var transpiredTime = DateTime.UtcNow - jobStoreSupport.lastCheckin;
            ...
            internal class MisfireHandler : QuartzThread
            {
                ...
                private RecoverMisfiredJobsResult Manage()
                {
                    try
                    {
                        ...
                        var res = jobStoreSupport.DoRecoverMisfires();
                        numFails = 0;
                        return res;
                    }
                    catch (Exception e)
                ...
                public override void Run()
                {
                    while (!shutdown)
                    {
                        ...
                        var recoverMisfiredJobsResult = Manage();
