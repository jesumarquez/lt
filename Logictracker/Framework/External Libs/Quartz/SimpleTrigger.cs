	...
	public class SimpleTrigger : Trigger
	{
        ...
        public override NullableDateTime FinalFireTimeUtc
        {
            get
            {
                ...
                var lastTrigger = StartTimeUtc.AddMilliseconds(repeatCount * repeatInterval.TotalMilliseconds);
		...
		public override void UpdateAfterMisfire(ICalendar cal)
		{
			...
			else if (instr == Quartz.MisfireInstruction.SimpleTrigger.RescheduleNextWithRemainingCount)
			{
				...
				if (newFireTime.HasValue)
				{
					var timesMissed = ComputeNumTimesFiredBetween(nextFireTimeUtc, newFireTime);
		...
		public override void UpdateWithNewCalendar(ICalendar calendar, TimeSpan misfireThreshold)
		{
            ...
            while (nextFireTimeUtc.HasValue && !calendar.IsTimeIncluded(nextFireTimeUtc.Value))
            {
                ...
                {
                    var diff = now - nextFireTimeUtc.Value;
        ...
        public override NullableDateTime GetFireTimeAfter(NullableDateTime afterTimeUtc)
		{
			...
			var startMillis = StartTimeUtc;
			var afterMillis = afterTimeUtc.Value;
			var endMillis = !EndTimeUtc.HasValue ? DateTime.MaxValue : EndTimeUtc.Value;
			...
			var numberOfTimesExecuted = (long) (((long) (afterMillis - startMillis).TotalMilliseconds / repeatInterval.TotalMilliseconds) + 1);
			...
			var time = startMillis.AddMilliseconds(numberOfTimesExecuted * repeatInterval.TotalMilliseconds);
        ...
        public virtual int ComputeNumTimesFiredBetween(NullableDateTime startTimeUtc, NullableDateTime endTimeUtc)
		{
			...
			var time = (long) (endTimeUtc.Value - startTimeUtc.Value).TotalMilliseconds;
