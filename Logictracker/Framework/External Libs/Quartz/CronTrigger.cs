...
namespace Quartz
{
	...
	public class CronTrigger : Trigger
	{
		...
		{
			var copy = (CronTrigger) MemberwiseClone();
			if (cronEx != null)
			{
		...
		public override DateTime StartTimeUtc
		{
			...
			set
			{
				...
				var dt = new DateTime(value.Year, value.Month, value.Day, value.Hour, value.Minute, value.Second);
        ...
        public override NullableDateTime GetFireTimeAfter(NullableDateTime afterTimeUtc)
		{
            ...
            var pot = GetTimeAfter(afterTimeUtc.Value);
		...
		public bool WillFireOn(DateTime test, bool dayOnly)
		{
            ...
            var fta = GetFireTimeAfter(test.AddMilliseconds(-1 * 1000));
#if !NET_35
            DateTime p = TimeZone.ToLocalTime(fta.Value);
#else
            var p = TimeZoneInfo.ConvertTimeFromUtc(fta.Value, TimeZone);
#endif
