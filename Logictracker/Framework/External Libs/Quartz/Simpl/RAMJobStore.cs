...
namespace Quartz.Simpl
{
	...
	public class RAMJobStore : IJobStore
	{
		...
		{
            var jw = new JobWrapper((JobDetail)newJob.Clone());
			...
			lock (triggerLock)
			{
				...
				{
					// update job detail
					var orig = (JobWrapper) jobsByFQN[jw.key];
		...
		{
			var key = JobWrapper.GetJobNameKey(jobName, groupName);

			var found = false;

			var trigger = GetTriggersForJob(ctxt, jobName, groupName);
		...
		{
            var tw = new TriggerWrapper((Trigger)newTrigger.Clone());
		...
		{
			var key = TriggerWrapper.GetTriggerNameKey(triggerName, groupName);
...
*/          
			...
			lock (triggerLock)
			{
                ...
                if (found)
                {
                    ...
                    var jw = (JobWrapper)jobsByFQN[JobWrapper.GetJobNameKey(tw.trigger.JobName, tw.trigger.JobGroup)];
                    var trigs = GetTriggersForJob(ctxt, tw.trigger.JobName, tw.trigger.JobGroup);
 ...
 */
		...
		public virtual bool ReplaceTrigger(SchedulingContext ctxt, string triggerName, string groupName, Trigger newTrigger)
		{
			...
			lock (triggerLock)
			{
				...
				tempObject = triggersByFQN[key];
				triggersByFQN.Remove(key);
				var tw = (TriggerWrapper) tempObject;
				found = tw != null;
		...
		{
			var jw = (JobWrapper) jobsByFQN[JobWrapper.GetJobNameKey(jobName, groupName)];
		...
		{
			var tw = (TriggerWrapper) triggersByFQN[TriggerWrapper.GetTriggerNameKey(triggerName, groupName)];
		                                  ...
		                                  bool updateTriggers)
		{
			...
			if (obj != null && updateTriggers)
			{
				lock (triggerLock)
				{
					var trigs = GetTriggerWrappersForCalendar(name);
					for (var i = 0; i < trigs.Count; ++i)
					{
						var tw = (TriggerWrapper) trigs[i];
						var trig = tw.Trigger;
						Boolean tempBoolean;
						tempBoolean = timeTriggers.Contains(tw);
						timeTriggers.Remove(tw);
						var removed = tempBoolean;
		...
		public virtual bool RemoveCalendar(SchedulingContext ctxt, string calName)
		{
			...
			lock (triggerLock)
			{
				...
				{
                    var trigg = triggerWrapper.Trigger;
		...
		public virtual string[] GetJobNames(SchedulingContext ctxt, string groupName)
		{
			...
			if (grpMap != null)
			{
				lock (triggerLock)
				{
					...
					var outListPos = 0;
					var keys = new HashSet(grpMap.Keys).GetEnumerator();
					while (keys.MoveNext())
					{
						var key = (string) keys.Current;
						var jw = (JobWrapper) grpMap[key];
						if (jw != null)
						{
		...
		{
			var names = new ArrayList(calendarsByName.Keys);
		...
		public virtual string[] GetTriggerNames(SchedulingContext ctxt, string groupName)
		{
			...
			if (grpMap != null)
			{
				lock (triggerLock)
				{
					...
					{
						var key = (string) keys.Current;
						var tw = (TriggerWrapper) grpMap[key];
						if (tw != null)
						{
		...
		public virtual string[] GetJobGroupNames(SchedulingContext ctxt)
		{
            ...
            lock (triggerLock)
			{
				...
				var outListPos = 0;
				var keys = new HashSet(jobsByGroup.Keys).GetEnumerator();
				while (keys.MoveNext())
				{
		...
		public virtual string[] GetTriggerGroupNames(SchedulingContext ctxt)
		{
			...
			lock (triggerLock)
			{
				...
				var outListPos = 0;
				var keys = new HashSet(triggersByGroup.Keys).GetEnumerator();
				while (keys.MoveNext())
				{
					outList[outListPos++] = ((string) keys.Current);
				}
		...
		public virtual Trigger[] GetTriggersForJob(SchedulingContext ctxt, string jobName, string groupName)
		{
			...
			var jobKey = JobWrapper.GetJobNameKey(jobName, groupName);
			lock (triggerLock)
			{
				for (var i = 0; i < triggers.Count; i++)
				{
					var tw = (TriggerWrapper) triggers[i];
		...
		public virtual void PauseTriggerGroup(SchedulingContext ctxt, string groupName)
		{
            lock (triggerLock)
			{
				...
				var names = GetTriggerNames(ctxt, groupName);
		...
		public virtual void PauseJob(SchedulingContext ctxt, string jobName, string groupName)
		{
            lock (triggerLock)
			{
				var t = GetTriggersForJob(ctxt, jobName, groupName);
		...
		public virtual void PauseJobGroup(SchedulingContext ctxt, string groupName)
		{
            lock (triggerLock)
			{
				...
				var jobNames = GetJobNames(ctxt, groupName);

				for (var i = 0; i < jobNames.Length; i++)
				{
					var t = GetTriggersForJob(ctxt, jobNames[i], groupName);
					for (var j = 0; j < t.Length; j++)
					{
						PauseTrigger(ctxt, t[j].Name, t[j].Group);
					}
		...
		public virtual void ResumeTriggerGroup(SchedulingContext ctxt, string groupName)
		{
            lock (triggerLock)
			{
				...
				for (var i = 0; i < names.Length; i++)
				{
                    var key = TriggerWrapper.GetTriggerNameKey(names[i], groupName);
				    if ((triggersByFQN[key] != null))
				    {
                        var jobGroup = ((TriggerWrapper) triggersByFQN[key]).Trigger.JobGroup;
		...
		public virtual void ResumeJobGroup(SchedulingContext ctxt, string groupName)
		{
            lock (triggerLock)
			{
				...
				for (var i = 0; i < jobNames.Length; i++)
				{
					var r = GetTriggersForJob(ctxt, jobNames[i], groupName);
		...
		public virtual void PauseAll(SchedulingContext ctxt)
		{
            lock (triggerLock)
			{
				var names = GetTriggerGroupNames(ctxt);
		...
		public virtual Trigger AcquireNextTrigger(SchedulingContext ctxt, DateTime noLaterThan)
		{
			...
			lock (triggerLock)
			{
				while (tw == null)
				{
					...
					var trig = (Trigger) tw.trigger.Clone();
					return trig;
				}
			}
		...
		public virtual void ReleaseAcquiredTrigger(SchedulingContext ctxt, Trigger trigger)
		{
			lock (triggerLock)
			{
				var tw = (TriggerWrapper) triggersByFQN[TriggerWrapper.GetTriggerNameKey(trigger)];
		...
		public virtual TriggerFiredBundle TriggerFired(SchedulingContext ctxt, Trigger trigger)
		{
			lock (triggerLock)
			{
				...
				var job = bndle.JobDetail;

				if (job.Stateful)
				{
					var trigs = GetTriggerWrappersForJob(job.Name, job.Group);
					var itr = trigs.GetEnumerator();
					while (itr.MoveNext())
					{
						var ttw = (TriggerWrapper) itr.Current;
				...
				{
                    var d = tw.trigger.GetNextFireTimeUtc();
                    if (d.HasValue)
					{
						lock (triggerLock)
						{
                                                 ...
                                                 SchedulerInstruction triggerInstCode)
		{
			lock (triggerLock)
			{
				var jobKey = JobWrapper.GetJobNameKey(jobDetail.Name, jobDetail.Group);
				var jw = (JobWrapper) jobsByFQN[jobKey];
				var tw = (TriggerWrapper) triggersByFQN[TriggerWrapper.GetTriggerNameKey(trigger)];
				...
				if (jw != null)
				{
					...
					if (jobDetail.Stateful)
					{
						...
						var trigs = GetTriggerWrappersForJob(jd.Name, jd.Group);
		...
		{
			var tws = GetTriggerWrappersForJob(jobName, jobGroup);
		...
		protected internal virtual string PeekTriggers()
		{
			...
			lock (triggerLock)
			{
				var itr = new HashSet(triggersByFQN.Keys).GetEnumerator();
				while (itr.MoveNext())
				{
			...
			lock (triggerLock)
			{
				var itr = timeTriggers.GetEnumerator();
				while (itr.MoveNext())
				{
	...
	internal class TriggerComparator : IComparer
	{
		...
		{
			var trig1 = (TriggerWrapper) obj1;
			var trig2 = (TriggerWrapper) obj2;

            var comp = trig1.trigger.CompareTo(trig2.trigger);
            if (comp != 0)
            {
                return comp;
            }
