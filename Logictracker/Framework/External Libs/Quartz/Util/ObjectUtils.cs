	...
	public sealed class ObjectUtils
	{
		...
		public static object ConvertValueIfNecessary(Type requiredType, object newValue)
		{
			if (newValue != null)
			{
                ...
                {
                    var t = Type.GetType(newValue.ToString());
                    if (t == null)
                    {
		...
		public static void SetObjectProperties(object obj, NameValueCollection props)
		{
			...
			{
				var propertyName = name.Substring(0, 1).ToUpper(CultureInfo.InvariantCulture) + name.Substring(1);
	    ...
	    {
            var attributes = pi.GetCustomAttributes(typeof(TimeSpanParseRuleAttribute), false);
            ...
            var attribute = (TimeSpanParseRuleAttribute)attributes[0];
