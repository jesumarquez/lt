        ...
        public override void Run()
        {
            ...
            while (!halted)
            {
                try
                {
                    ...
                    var availTreadCount = qsRsrcs.ThreadPool.BlockForAvailableThreads();
                    if (availTreadCount > 0) // will always be true, due to semantics of blockForAvailableThreads...
                    {
                        ...
                        {
                            now = DateTime.UtcNow;
                            var triggerTime = trigger.GetNextFireTimeUtc().Value;
                    ...
                    var waitTime = utcNow.Add(GetRandomizedIdleWaitTime());
                    var timeUntilContinue = waitTime - utcNow;
                    lock (sigLock) 
                    {
                	    try 
                        {
        ...
        {    	
            ...
            {	
			    ...
			    if(earlier) 
                {
                    ...
                    var diff = oldTimeUtc - DateTime.UtcNow;
