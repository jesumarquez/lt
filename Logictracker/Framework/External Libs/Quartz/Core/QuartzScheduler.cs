...
namespace Quartz.Core
{
    ...
    public class QuartzScheduler : MarshalByRefObject, IRemotableQuartzScheduler
    {
        ...
        internal ExecutingJobsManager jobMgr;
        internal ErrorLogger errLogger;
        ...
        private NullableDateTime initialStart;
        ...
        public void StartDelayed(TimeSpan delay)
        {
            ...
            var starter = new DelayedSchedulerStarter(this, delay, Log);
            var t = new Thread(starter.Run);
            t.Start();
        }
        ...
        public virtual DateTime ScheduleJob(SchedulingContext ctxt, JobDetail jobDetail, Trigger trigger)
        {
            ...
            var ft = trigger.ComputeFirstFireTimeUtc(cal);
        ...
        public virtual NullableDateTime RescheduleJob(SchedulingContext ctxt, string triggerName, string groupName, Trigger newTrigger)
        {
            ...
            var ft = newTrigger.ComputeFirstFireTimeUtc(cal);
        ...
        {
            var listeners = GlobalTriggerListeners;
            for (var i = 0; i < additionalListeners.Length; i++)
            {
                var tl = GetTriggerListener(additionalListeners[i]);
        ...
        {
            var listeners = GlobalJobListeners;
            for (var i = 0; i < additionalListeners.Length; i++)
            {
                var jl = GetJobListener(additionalListeners[i]);
        ...
        public virtual bool NotifyTriggerListenersFired(JobExecutionContext jec)
        {
            ...
            var listeners = BuildTriggerListenerList(jec.Trigger.TriggerListenerNames);
            ...
            foreach (ITriggerListener tl in listeners)
            {
                ...
                {
                    var se = new SchedulerException(string.Format(CultureInfo.InvariantCulture, "TriggerListener '{0}' threw exception: {1}", tl.Name, e.Message), e);
        ...
        {
            // build a list of all trigger listeners that are to be notified...
            var listeners = BuildTriggerListenerList(trigger.TriggerListenerNames);
        ...
        public virtual void NotifyJobListenersToBeExecuted(JobExecutionContext jec)
        {
            ...
            var listeners = BuildJobListenerList(jec.JobDetail.JobListenerNames);
            ...
            foreach (IJobListener jl in listeners)
            {
                ...
                {
                    var se = new SchedulerException(string.Format(CultureInfo.InvariantCulture, "JobListener '{0}' threw exception: {1}", jl.Name, e.Message), e);
        ...
        public virtual void NotifySchedulerListenersError(string msg, SchedulerException se)
        {
            ...
            var schedListeners = SchedulerListeners;
