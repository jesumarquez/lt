...
namespace Urbetrack.Compresion
{
    public class Huffman
    {
        ...
        public byte[] Encode()
        {
            ...
            var msEncodedOutput = new MemoryStream();
            ...
            var msEncodedHeader = new MemoryStream();
            foreach (var l in OptimizedTree)
            {
            ...
            iBuffer = 0;
            iBufferCount = 0;
            foreach (var b in msInputbytes.ToArray())
            {
                var FoundLeaf = OptimizedTree[b];
                ...
                {
                    //Write to output
                    var iBufferOutput = (int)(iBuffer >> (iBufferCount - 8));
        ...
        private bool EncodeTree()
        {
            ...
            foreach (var Node in HuffmanTree)
            {
                ...
                if (!Node.IsNode && Node.FrequencyValue != 0)
                {
                    ...
                    if (Node.Left || Node.Right)
                    {
                        ...
                        {
                            iBinaryValue ^= (1 << iBitCount);
                            iBitCount++;
                        }

                        //Process up the tree through the parent nodes
                        var ParentNode = HuffmanTree.Find(delegate(Leaf leaf) { return leaf.ID == Node.ParentID; });
                    ...
                    if (iBinaryValue != 0) iLeadingZeros = iBitCount - Convert.ToString(iBinaryValue, 2).Length;
        ...
        {
            if (!Hacker.QTree.HuffmanEnabled)
            {
                // para que nunca lo elija, es mas grade que la fuente.
                return new byte[src_size+1];
            }
            var b2 = new byte[src_size];
            Array.Copy(source, offset, b2, 0, src_size);
