...
namespace RNR.Maps
{
    ...
    internal sealed class TilePanel : Canvas
    {
        ...
        public bool RequiresUpdate
        {
            get
            {
                var r = ForceUpdate || _baseTile == null ||
                       (_baseTile.TileX + 1) - this.LeftTile != 0 || // The baseTile is loated at -1,-1 but LeftTile/TopTile is 0,0
        ...
        public void Update(bool regenerate)
        {
            if (_baseTile == null || regenerate)
            {
                this.RegenerateTiles(true);
            }
            else
            {
        ...
        /// <param name="processEvents">Determins whether to call DoEvents after every row has been created.</param>
        /// <remarks>Calling DoEvents inside an event handler (such as OnRenderSizeChanged) causes an infinite loop.</remarks>
        private void RegenerateTiles(bool processEvents)
        {
            this.Children.Clear();
            for (int x = -1; x < _columns; ++x)
            {
                if (processEvents)
                {
                    DoEvents(); // Pause this work to allow UI to complete
                }
