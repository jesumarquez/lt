...
namespace Logictracker.Scheduler.Tasks.LoJack
{
    public class Task : BaseTask
    {
        ...
        protected override void OnExecute(Timer timer)
        {
            ...
            {
                if (empresa.LoJackApiHabilitado)
                {
                    var user = empresa.LoJackApiUsuario;
                    var password = empresa.LoJackApiPassword;
                    var guid = empresa.LoJackApiGuid;

                    STrace.Trace(ComponentName, string.Format("LoJack API Habilitada para: {0}", empresa.RazonSocial));
                    ...
                    try
                    {
                        ...
                        if (resultado.Valido && resultado.Entidades != null)
                        {
                            ...
                            foreach (var ultimoSuceso in ultimosSucesos)
                            {
                                ...
                                var coche = DaoFactory.CocheDAO.FindByPatente(empresa.Id, patente);
                                if (coche != null)
                                {
                ...
                {
                    STrace.Trace(ComponentName, string.Format("LoJack API No Habilitado para: {0}", empresa.RazonSocial));
                }
            }
        }
